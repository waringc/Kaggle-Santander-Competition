#########################
#This script creates an XGBoost model using the "logistic:binary" objective
...
#########################
#generate empty data frames to store out of fold predictions
#and test predictions(allpredictions)
...
#########################
#Run model, using k-folds and creating out of fold predictions
i=1
kfolds = 10
for (category in category_names){
      ...
      #Swapped between objectives
      #used both "binary:logistic" and 'multi:softprob'
      xgb_params = list(
        ...
        min_child_weight= 2,
        eval_metric= 'logloss',
        base_score=sum(y_use)/length(y_use)
      )
      
      folds<-createFolds(y_use, k = kfolds, list = TRUE, returnTrain = FALSE)
      ...
      for (fold in folds){
              ...
              #perform prediction
              allpredictions[i]=allpredictions[i] + as.data.frame(matrix(predict(gbdt,dtest), nrow=dim(xtest), byrow=TRUE))
              
              
              out_of_fold[fold,i] <- as.data.frame(matrix(predict(gbdt,dval), nrow=dim(x_val), byrow=TRUE))
      }
i=i+1   
}
...
save(allpredictions,out_of_fold,file="Dec18-BinaryClass-10Fold-1066Seed.Rdata")
